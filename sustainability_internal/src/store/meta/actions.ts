...
export const actions: ActionTree<MetaState, RootState> = {
  ...
  unlockLessonPart(
    ...
    payload: { lesson: string; part: number }
  ) {
    // less on is intro, whatsInScope, exploringScope3, dayInTheLife
    const { lesson, part } = payload
    const { lockedLessonParts } = state.scorm.suspendedData
    const lockedArray = [...lockedLessonParts[lesson]]
    lockedArray.splice(part, 1, false)
    lockedLessonParts[lesson] = lockedArray
